classdef Links < handle
    ...
    methods 
        ...
        % 
        % function Jp = set_jacobian(this)
        %     Jp = sym(zeros(6,3));
        %     for i = 1: this.index
        % 
        %         Z = this.robot.get_z_jacobian(i);
        %         if this.robot.links{i}.name == "rec"
        %             column = [Z;0;0;0];
        % 
        % 
        %         else
        %             upper = cross(Z,this.com_base - this.robot.links{i}.prev_joint_basef); % cross(unit_vector of axis z of frame j-1 wrt base, com wrt base - position vector of the origin frame j-1 wrt base
        %             column = [upper; Z];
        %         end
        %         Jp(:,i) = column;
        % 
